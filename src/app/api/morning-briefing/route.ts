...
async function handler() {
  try {
    ...
    const hotLeads = calls?.filter(c => c.ai_summary?.toLowerCase().includes('lead') || c.disposition === 'hot_lead') || []
    const missedCalls = calls?.filter(c => !c.duration || c.duration === 0) || []
    ...
    const emailHtml = `
      <div style="font-family:sans-serif;max-width:700px;margin:0 auto;padding:24px">
        ...
        ${hotLeads.length > 0 ? `<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:8px;padding:16px;margin-bottom:24px"><h3 style="margin:0 0 12px;color:#d97706">Hot Leads Requiring Follow-up</h3>${hotLeads.map((l: any) => `<p style="margin:4px 0">- ${l.caller_id_number}: ${l.ai_summary?.substring(0, 100) || 'No summary'}</p>`).join('')}</div>` : ''}
        ...
        <p style="color:#94a3b8;font-size:12px;margin-top:32px;text-align:center">UnifyLine by IntelSys Technologies</p>
      </div>
    `
