...
// Per-account recipient overrides. The IntelSys/default account keeps using
// the existing ALERT_EMAIL env var (unchanged behavior). Add other accounts
// here as they're onboarded, or move this to a proper `notification_email`
// column on the `accounts` table in a future pass.
const ACCOUNT_RECIPIENTS: Record<string, string> = {
  // MTI Premium Charters - using their existing SEO-project Gmail for now.
  // PLACEHOLDER: swap for their real production inbox once confirmed.
  '5f646bc2-9bf3-47cb-9859-2871d4322e19': 'mtiprojects7@gmail.com',
}

const DEFAULT_ACCOUNT_ID = '4144d882-4fa2-4fbe-9264-b50af24ad5ed' // IntelSys
...
async function handler() {
  try {
    // Find every account that has any module enabled - each gets its own
    // briefing. (Today this is effectively "every provisioned account";
    // a `morning_briefing` opt-out flag can be added to account_modules
    // later if some accounts shouldn't get one.)
    const { data: accountRows } = await supabase
      .from('accounts')
      .select('id, name')
      .eq('status', 'active')

    // Trial accounts (like MTI right now) still get briefings during
    // evaluation/demo - include those too.
    const { data: trialRows } = await supabase
      .from('accounts')
      .select('id, name')
      .eq('status', 'trial')

    const accounts: AccountInfo[] = [...(accountRows || []), ...(trialRows || [])]

    if (accounts.length === 0) {
      // Fallback: no accounts table data somehow - send the legacy
      // un-scoped briefing exactly as before, to ALERT_EMAIL.
      const result = await sendBriefingForAccount(null, process.env.ALERT_EMAIL || '', 'UnifyLine')
      return NextResponse.json({ success: true, sent: 1, results: [result] })
    }

    const results = []
    for (const account of accounts) {
      const recipient = ACCOUNT_RECIPIENTS[account.id] || process.env.ALERT_EMAIL || ''
      if (!recipient) {
        results.push({ account: account.name, skipped: true, reason: 'no recipient configured' })
        continue
      }
      const result = await sendBriefingForAccount(account.id, recipient, account.name)
...
async function sendBriefingForAccount(accountId: string | null, recipientEmail: string, accountName: string) {
  ...
  // Scope to this account only. If accountId is null (legacy fallback path),
  // pull everything exactly as the original un-scoped behavior did.
  if (accountId) {
...
<div style="max-width:680px;margin:24px auto;padding:0 16px">
  ...
  <div style="background:linear-gradient(135deg,#0f2744 0%,#1a4480 100%);padding:28px 32px;border-radius:12px 12px 0 0">
    <div style="display:flex;justify-content:space-between;align-items:center">
      <div>
        <h1 style="color:white;margin:0;font-size:22px;font-weight:700;letter-spacing:-0.5px">${accountName}</h1>
        <p style="color:#93c5fd;margin:4px 0 0;font-size:13px">Morning Briefing &middot; ${dateStr}</p>
      </div>
  ...
  <p style="text-align:center;color:#94a3b8;font-size:12px;margin-top:20px">
    UnifyLine by IntelSys Technologies &middot; <a href="https://www.unifyline.com" style="color:#94a3b8">unifyline.com</a>
  </p>
</div>
</body>
</html>
  ...
  if (process.env.RESEND_API_KEY && recipientEmail) {
    ...
    await resend.emails.send({
      ...
      to: recipientEmail,
      subject: `${accountName} Morning Briefing · ${totalCalls} calls, ${hotLeads.length} leads · ${new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`,
      html: emailHtml,
    })
  }
